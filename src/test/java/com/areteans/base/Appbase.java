...
import com.areteans.objrepo.Locator;
import com.areteans.objrepo.ObjectRepository;
...
public class Appbase implements ObjectRepository {
	...
	protected boolean displayedAndVisibleOnPage(final Locator locator) {
		try {
			...
			FluentWait<WebDriver> wait = new FluentWait<WebDriver>(getDriver()).withTimeout(Duration.ofSeconds(GlobalVariables.maxTimeLoad))
			...
			if (!value.trim().isEmpty()) {
				if (locatorEnabled(locator)) {
					...
					try {
						...
						ReporterClass.writeToReport("fail",
								"Value " + value + " NOT selected from " + locator.getName());
					}
	...
	protected void openURL() {
		getDriver().get(Config.getProperty("appUrl"));
	}
	...
	public static void takeScreenshot() {
		...
		try {
			FileHandler.copy(src, new File(GlobalVariables.currentscreenshotPath + "//"
					+ GlobalVariables.currentTestCaseName + "_" + dateFormat.format(now) + ".png"));
